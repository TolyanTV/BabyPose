...
class SegTrainSemi(data.Dataset):
    def __init__(self, data_dir, mirror, color_jitter, rotate, mean, num_examples):
        ...
        self.image_dir = os.path.join(data_dir, "tl_images")
    ...
    def __getitem__(self, idx):
        ...
        ##################################################################################################################
        #src_img_name = self.train_label_dict[chosen_vid_name][src_frame_idx]["image"]
        #src_img_path = os.path.join(self.image_dir, src_img_name)
        #src_img_arr = cv2.imread(src_img_path, cv2.IMREAD_COLOR)
        #if src_img_arr is None:
            #raise FileNotFoundError(f"Image not found: {src_img_path}")
        #tar_img_name = self.train_label_dict[chosen_vid_name][tar_frame_idx]["image"]
        #tar_img_path = os.path.join(self.image_dir, tar_img_name)
        #tar_img_arr = cv2.imread(tar_img_path, cv2.IMREAD_COLOR)
        #if tar_img_arr is None:
            #raise FileNotFoundError(f"Image not found: {tar_img_path}")
        src_img_path = os.path.join(self.image_dir, self.train_label_dict[chosen_vid_name][src_frame_idx]["image"])
        src_img_arr = cv2.imread(src_img_path, cv2.IMREAD_COLOR)
        tar_img_path = os.path.join(self.image_dir, self.train_label_dict[chosen_vid_name][tar_frame_idx]["image"])
        tar_img_arr = cv2.imread(tar_img_path, cv2.IMREAD_COLOR)
        ...
        #unlabel_src_img_name = self.train_unlabel_dict[chosen_vid_name][unlabel_src_frame_idx]["image"]
        #unlabel_src_img_path = os.path.join(self.image_dir, unlabel_src_img_name)
        #unlabel_src_img_arr = cv2.imread(unlabel_src_img_path, cv2.IMREAD_COLOR)
        #if unlabel_src_img_arr is None:
            #raise FileNotFoundError(f"Image not found: {unlabel_src_img_path}")
        #unlabel_tar_img_name = self.train_unlabel_dict[chosen_vid_name][unlabel_tar_frame_idx]["image"]
        #unlabel_tar_img_path = os.path.join(self.image_dir, unlabel_tar_img_name)
        #unlabel_tar_img_arr = cv2.imread(unlabel_tar_img_path, cv2.IMREAD_COLOR)
        #if unlabel_tar_img_arr is None:
            #raise FileNotFoundError(f"Image not found: {unlabel_tar_img_path}")
        #unlabel_src_img_path = os.path.join(self.image_dir, self.train_unlabel_dict[chosen_vid_name][unlabel_src_frame_idx]["image"])
        #unlabel_src_img_arr = cv2.imread(unlabel_src_img_path, cv2.IMREAD_COLOR)
        #unlabel_tar_img_path = os.path.join(self.image_dir, self.train_unlabel_dict[chosen_vid_name][unlabel_tar_frame_idx]["image"])
        #unlabel_tar_img_arr = cv2.imread(unlabel_tar_img_path, cv2.IMREAD_COLOR)
...
class SegTest(data.Dataset):
    def __init__(self, data_dir, mean):
        ...
        self.image_dir = os.path.join(data_dir, "testimages")
        self.files = []
    ...
    def __getitem__(self, idx):
        ...
        return image.copy(), label.copy(), img_name
